...
export const MomPlantList = () => {
    ...
    const { momPlants, getMomPlant, removeMomPlant } = useContext(MomPlantContext);
        ...
        <>
            <Container className="bg-light overflow-auto h-15 border border-primary rounded-top">
                ...
                <Button
                    outline
                    color="primary"
                    className="float-right sticky-top"
                    onClick={() => {
                        history.push('/momPlants/create');
                    }}>
                    Add Mom Plant
                </Button>
            </Container>
            <Container className="bg-light overflow-auto h-50 border border-primary rounded-bottom border-top-0">
                {momPlants?.map((momPlant) => {
                    ...
                    return <MomPlantCard key={momPlant.id} momPlant={momPlant} removeMomPlant={removeMomPlant} />;
                })}
            </Container>
        </>
    );
